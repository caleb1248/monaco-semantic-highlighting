...
import { Uri, worker } from "typed-monaco-editor-core";
...
export class TypeScriptWorker implements ts.LanguageServiceHost, ITypeScriptWorker {
  ...
  async getSemanticClassifications(fileName: string): Promise<ts.Classifications | undefined> {
    const file = this._getScriptText(fileName);
    if (!file) {
      return undefined;
    }
