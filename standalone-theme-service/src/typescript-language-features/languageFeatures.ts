...
{
  constructor(worker: (...uris: Uri[]) => Promise<TypeScriptWorker>) {
    super(worker);
  }
  tokenTypes: string[] = [
    "class",
  ...
  async provideDocumentSemanticTokens(
    ...
    const result = await worker.getSemanticClassifications(fileName);
    ...
    let prevPosition = model.getPositionAt(0);

    for (let i = 0; i < spans.length; ) {
      const offset = model.getPositionAt(spans[i++]);
      const length = spans[i++];
      const classification = spans[i++];

      // classification = (type + 1) << 8 + modifiers
      const tokenType = classification < 255 ? undefined : (classification >> 8) - 1;
      const tokenModifiers = classification & 255;

      const modifierArray = [];
      for (let i = 0; i < 8; i++) {
        if (tokenModifiers & (2 ** i)) modifierArray.push(this.tokenModifiers[i]);
      }
      ...
      data.push(
        ...
        modifierArray
          ...
          .reduce((a, b) => a | (2 ** this.tokenModifiers.indexOf(b)), 0)
      );
