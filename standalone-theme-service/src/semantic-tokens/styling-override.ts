...
import {
  ...
  SemanticTokens,
} from "monaco-editor-core/esm/vs/editor/common/languages.js";
import { TokenMetadata } from "monaco-editor-core/esm/vs/editor/common/encodedTokenAttributes.js";
import { IThemeService } from "monaco-editor-core/esm/vs/platform/theme/common/themeService.js";
import { ILogService, LogLevel } from "monaco-editor-core/esm/vs/platform/log/common/log.js";
import { SparseMultilineTokens } from "monaco-editor-core/esm/vs/editor/common/tokens/sparseMultilineTokens.js";
import { ILanguageService } from "monaco-editor-core/esm/vs/editor/common/languages/language.js";
import * as original from "monaco-editor-core/esm/vs/editor/common/services/semanticTokensProviderStyling.js";
import { convertToken } from "./index";
import { parseClassifierString } from "monaco-editor-core/esm/vs/platform/theme/common/tokenClassificationRegistry";
import {
  IStandaloneTheme,
  ...
  public getMetadata(tokenTypeIndex: number, tokenModifierSet: number, languageId: string): number {
    ...
    if (entry) {
      ...
      if (tokenType) {
        ...
        const colorTheme = this._themeService.getColorTheme() as IStandaloneTheme & {
          themeData: IStandaloneThemeData;
        };

        let tokenStyle = colorTheme.getTokenStyleMetadata(tokenType, tokenModifiers, languageId);

        let token = [tokenType, ...tokenModifiers].join(".");
        console.log("token", token);
        let matchingToken = colorTheme.themeData.rules.find((rule) => rule.token === token);
        if (typeof matchingToken === "undefined") {
          console.log("override");
          const converted = parseClassifierString(
            convertToken([tokenType, ...tokenModifiers].join("."), languageId)
          );
          tokenStyle = this._themeService
            .getColorTheme()
            .getTokenStyleMetadata(converted.type, converted.modifiers, languageId);
        }
        if (typeof tokenStyle === "undefined") {
          ...
          if (typeof tokenStyle.italic !== "undefined") {
            const italicBit =
              (tokenStyle.italic ? FontStyle.Italic : 0) << MetadataConsts.FONT_STYLE_OFFSET;
            metadata |= italicBit | MetadataConsts.SEMANTIC_USE_ITALIC;
          }
          if (typeof tokenStyle.bold !== "undefined") {
            const boldBit =
              (tokenStyle.bold ? FontStyle.Bold : 0) << MetadataConsts.FONT_STYLE_OFFSET;
            metadata |= boldBit | MetadataConsts.SEMANTIC_USE_BOLD;
          }
          if (typeof tokenStyle.underline !== "undefined") {
            const underlineBit =
              (tokenStyle.underline ? FontStyle.Underline : 0) << MetadataConsts.FONT_STYLE_OFFSET;
...
// @ts-ignore
original.SemanticTokensProviderStyling.prototype.getMetadata = function (
  tokenTypeIndex: number,
  tokenModifierSet: number,
  languageId: string
) {
  return SemanticTokensProviderStyling.prototype.getMetadata.bind(this)(
    tokenTypeIndex,
    tokenModifierSet,
    languageId
  );
